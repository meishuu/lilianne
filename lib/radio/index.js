...
function skipRatio(length) {
  ...
  return 0.6 - 0.3 / (1 + Math.exp(3 - minutes / 3));
}
...
module.exports = class Radio extends events.EventEmitter {
  ...
  addDj(user) {
    if (this.order.some(u => u.equals(user))) return;

    if (!this.current) {
      this.order.push(user);
    } else {
      let last = this.order.pop();
      if (last && !last.equals(this.current.user)) {
  ...
  removeDj(user) {
    const idx = this.order.findIndex(u => u.equals(user));
    if (idx !== -1) {
  ...
  addSong(link, user) {
    ...
    handler.getMeta(function onMeta(err, song) {
      ...
      function getFile(fp, cb) {
        function download() {
          mkdirp(cache, function onMkdirp(err) {
            if (err) return cb(err);

            handler.download(fs.createWriteStream(fp))
              .on('error', function onError(err) { cb(err); })
              .on('finish', function onFinish() { cb(null, true); });

            emitter.emit('downloading');
          });
        }

        fs.stat(fp, function onStat(err, stats) {
          // not cached
          if (err) {
            if (err.code === 'ENOENT') {
              download();
      ...
      async.parallel([
        ...
        if (!data) {
          ...
          replaygain(fp, function onReplayGain(err, gain) {
            if (err) return emitter.emit('error', err);
